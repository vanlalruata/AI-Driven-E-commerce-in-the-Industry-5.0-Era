...
The routing logic utilizes calibrated probabilities $P(pos)$ and Gini concentration coefficients ($G_{SKU} = 0.6843$, $G_{Region} = 0.8169$) to allocate human and automated resources:
\begin{enumerate}
    \item \textbf{Path A: Automated Logging}: For reviews with $P(pos) \ge 0.6$ (highly likely positive), the system logs the review to the standard batch dashboard. This requires no human intervention, conserving operational resources.
    \item \textbf{Path B: Human-in-the-Loop Buffer}: For reviews falling in the ambiguous range $0.4 < P(pos) < 0.6$, where prediction uncertainty is highest, the review is routed to the human review pool. This prevents misclassifying minority class (negative) sentiment on critical items, preserving human agency as mandated by Industry 5.0.
    \item \textbf{Path C: Standard CRM Ticketing}: For reviews with $P(pos) \le 0.4$ on low-concentration products ($G_{SKU} \le 0.6843$), an automated refund/CRM ticket is generated.
    \item \textbf{Path D: Immediate Supply Escalation}: For reviews with $P(pos) \le 0.4$ on high-concentration products ($G_{SKU} > 0.6843$), the review bypasses automated CRM and triggers an immediate email alert to the supply chain manager to check for defective batches.
    \item \textbf{Path E: Regional Inventory Reallocation}: For reviews with $P(pos) \le 0.4$ in high-revenue concentrated regions ($G_{Region} > 0.8169$), a notification is dispatched to the logistics coordinator to review regional storage and transit conditions.
\end{enumerate}
